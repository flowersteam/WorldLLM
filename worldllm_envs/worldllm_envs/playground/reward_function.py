...
def get_extra_grow_descriptions(
    get_supply_contact_ids,
    initial_state,
    current_state,
    params,
    obj_attributes,
    ...
    verb = "Attempted grow"
    grow_descriptions = []
    list_exluded = params["categories"]["living_thing"] + (
        "living_thing",
        "animal",
        "plant",
    )
...
def sample_descriptions_from_state(state, params):
    ...
    get_supply_contact = params["extract_functions"]["get_interactions"][
        "get_supply_contact"
    ]
    ...
    get_agent_position_attributes = params["extract_functions"][
        "get_agent_position_attributes"
    ]
    ...
    def sort_attributes(attributes):
        ...
        for att in attributes:
            if (
                att
                in tuple(params["categories"].keys()) + params["attributes"]["types"]
            ):
    ...
    if "Grasp" in admissible_actions:
        descriptions += get_grasp_descriptions(
            get_grasped_ids,
            current_state,
            sort_attributes,
            obj_attributes,
    ...
    if "Grow" in admissible_actions:
        descriptions += get_grow_descriptions(
            get_grown_ids,
            initial_state,
            current_state,
            params,
...
def get_reward_from_state(state, goal, params):
    ...
    if words[0] == "Grasp":
        grasp_descr = get_grasp_descriptions(
            get_grasped_ids,
            current_state,
            sort_attributes,
            obj_attributes,
    ...
    # Add Grow descriptions
    if words[0] == "Grow":
        grow_descr = get_grow_descriptions(
            get_grown_ids,
            initial_state,
            current_state,
            params,
