...
from worldllm_envs.playground.color_generation import *
...
def get_env_params(
    ...
    carnivorous = ("lion", "grizzly", "shark", "fox", "bobcat", "coyote")
    small_carnivorous = ("fox", "bobcat", "coyote")
    big_carnivorous = ("lion", "grizzly", "shark")
    herbivorous = ("elephant", "giraffe", "rhinoceros", "pig", "cow", "sheep")
    ...
    types = tuple(
        OrderedDict.fromkeys(types)
    )  # filters doubles, when some categories include others.
    nb_types = len(types)
    ...
    words_test_set_def = (
        tuple(f"{c} potato" for c in colors + ("any",))
    ...
    def get_obj_features(state, i_obj):
        inds = np.arange(
            dim_body_features + dim_obj_features * i_obj,
            dim_body_features + dim_obj_features * (i_obj + 1),
        )
        return state[inds]
    ...
    def get_darkest_obj_id(all_obj_features):
        ...
        shades = np.array(
            tuple(feature[color_inds].mean() for feature in all_obj_features)
        )
    ...
    def get_leftest_obj_id(all_obj_features):
        # list of obj_features
        x_position = np.array(
            tuple(feature[position_inds[0]] for feature in all_obj_features)
        )
    ...
    def get_highest_obj_id(all_obj_features):
        # list of obj_features
        y_position = np.array(
            tuple(feature[position_inds[1]] for feature in all_obj_features)
        )
    ...
    get_attributes_functions = dict(
        relative_shades=get_obj_relative_shades,
    ...
    def get_agent_position_attributes(state):
        ...
        if agent_pos[0] < -0.25:
            ...
            if (
                agent_pos[1] < 0.25
                and agent_pos[1] > -0.25
                and agent_pos[0] < 0.25
                and agent_pos[0] > -0.25
            ):
                out.append("center")
            ...
            for i_supply in supply_ids:
                for i_non_living in non_living_thing_ids:
                    if i_supply != i_non_living:
                        sizes = [
                            all_obj_features[i_supply][size_inds],
                            all_obj_features[i_non_living][size_inds],
                        ]
                        positions = [
                            all_obj_features[i_supply][position_inds],
                            all_obj_features[i_non_living][position_inds],
                        ]
                        if (
    ...
    get_interactions = dict(
        get_touched=get_touched_obj_ids,
    ...
    def combine_two(attribute_a, attribute_b):
        ...
        for a in attribute_a:
            for b in attribute_b:
                ...
                if (
                    a != b
                    and not check_equal_cat(a, b)
                    and not check_if_relative(a)
                    and not check_if_relative(b)
                ):
                    att_combinations.append("{} and {}".format(a, b))
        return tuple(att_combinations)

    params["extract_functions"] = dict(
