...
from worldllm_envs.playground.objects import generate_objects
...
class PlayGroundNavigationV1(gym.Env):
    ...
    def __init__(
        ...
        max_nb_objects=8,  # number of objects in the scene,min: 7 to cover all goals
        random_nb_obj=False,
        admissible_actions=(
            "Move",
            "Grasp",
        ...
        min_max_sizes=(
            (0.2, 0.25),
            (0.25, 0.3),
        ),  # ranges of sizes of objects (small and large ones)
        ...
        self.adm_abs_attributes = [
            a for a in self.adm_attributes if "relative" not in a
        ]
        ...
        self.inds_objs = [
            np.arange(
                self.dim_body + self.dim_obj * i_obj,
                self.dim_body + self.dim_obj * (i_obj + 1),
            )
        ...
        self.action_space = spaces.Box(
            low=-np.ones(self.dim_act), high=np.ones(self.dim_act), dtype=np.float32
        )
        self.observation_space = spaces.Box(
            low=-np.ones(self.dim_obs), high=np.ones(self.dim_obs), dtype=np.float32
        )
        ...
        if self.render_mode == "human":
            ...
            if self.reward_screen:
                self.viewer = pygame.display.set_mode(
                    (self.screen_size + 300, self.screen_size)
                )
            else:
                self.viewer = pygame.display.set_mode(
    ...
    def reset_with_goal(self, goal_str):
        ...
        for goal in goals:
            ...
            if words[0].lower() == "grow":
                obj_to_be_grown = dict(
                    zip(
                        self.adm_abs_attributes,
                        [None for _ in range(len(self.adm_abs_attributes))],
                    )
                )
                obj_supply = dict(
                    zip(
                ...
                for w in words[1:]:
                    for k in self.adm_abs_attributes:
                        if w in self.attributes[k]:
                            obj_to_be_grown[k] = w
                if (
                    obj_to_be_grown["categories"] is None
                    and obj_to_be_grown["types"] is None
                ):
                    # if only attributes are proposed, sample a grownable object type
                    obj_to_be_grown["categories"] = np.random.choice(
                        [
                            "small_herbivorous",
                            "big_herbivorous",
                            "small_carnivorous",
                ...
                if obj_to_be_grown["types"] in self.categories["plant"]:
                ...
                elif obj_to_be_grown["types"] in self.categories["small_herbivorous"]:
                    obj_supply.update(dict(categories="supply", types="water"))
                    objs.append(obj_supply.copy())
                    obj_supply = dict(
                        zip(
                    ...
                    objs.append(obj_supply.copy())
                elif obj_to_be_grown["types"] in self.categories["big_herbivorous"]:
                    for _ in range(2):
                        obj_supply.update(dict(categories="supply", types="water"))
                        objs.append(obj_supply.copy())
                        obj_supply = dict(
                ...
                elif obj_to_be_grown["types"] in self.categories["small_carnivorous"]:
                    obj_supply.update(dict(categories="supply", types="water"))
                    objs.append(obj_supply.copy())
                    obj_supply = dict(
                        zip(
                    ...
                    if np.random.uniform() < 0.5:
                        obj_supply.update(dict(categories="big_herbivorous"))
                        objs.append(obj_supply.copy())
                        obj_supply = dict(
                            zip(
                                self.adm_abs_attributes,
    ...
    def reset_scene(self, objects=None):
        ...
        if self.random_init:
            self.agent_pos += np.random.uniform(
                -self.agent_initial_pos_range, self.agent_initial_pos_range, 2
            )
        ...
        for obj in self.objects:
            ...
            self.object_grasped, rm_obj = obj.update_state(
                self.agent_pos,
    ...
    def step(self, action):
        ...
        self.observation[: self.half_dim_obs] = self.observe()
        self.observation[self.half_dim_obs :] = (
            self.observation[: self.half_dim_obs] - self.initial_observation
        )
    ...
    def render(self, goal_str="", mode="human", close=False):
        ...
        gripper_icon = pygame.transform.scale(
            gripper_icon, (size_gripper_pixels, size_gripper_pixels)
        ).convert_alpha()
        closed_gripper_icon = pygame.image.load(
            self.params["img_path"] + "hand_closed.png"
        )
        closed_gripper_icon = pygame.transform.scale(
            closed_gripper_icon, (size_gripper_closed_pixels, size_gripper_pixels)
        ).convert_alpha()
