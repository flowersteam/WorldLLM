...
def generate_all_descriptions(env_params):
    ...
    if "Grow" in p["admissible_actions"]:
        list_exluded = p["categories"]["living_thing"] + (
            "living_thing",
            "animal",
            "plant",
        )
                ...
                if not check_if_relative(adj):
                    for name in name_attributes:
                        if name not in list_exluded:
                            ...
                            attempted_grow_descriptions.append(
                                "Attempted grow {} {}".format(adj, name)
                            )
                attempted_grow_descriptions.append(
                    "Attempted grow {} {} thing".format(quantifier, adj)
                )
        for name in name_attributes:
            if name not in list_exluded:
    ...
    for descr in all_descriptions:
        ...
        for w in p[
            "words_test_set_def"
        ]:  # words_test_set_def is the set of occurrences that is reserved to the testing set.
            if w in descr:
                to_remove = True
                break
        if not to_remove:
...
if __name__ == "__main__":
    ...
    train_descriptions, test_descriptions, extra_descriptions = (
        generate_all_descriptions(env_params)
    )
