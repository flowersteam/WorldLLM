...
def compute_reward(
    ...
    """Generate the reward for the agent
    ...
    Args:
        ...
        agent (SB3Agent): The agent object that interacts with the environment.
        best_rule (str): The current best rule.
        prev_best_rule (str): The previous best rule.
        prompt_trajectories (List[Trajectory]): List of trajectories generated by the agent.

    Raises:
        ValueError: If the reward type specified in the configuration is unknown.

    Returns:
        np.ndarray: The computed rewards for the agent.
    """
    ...
    for i in tqdm(
        ...
        if isinstance(agent, SB3Agent):
            ...
            new_rewards = compute_reward(
                cfg, statistician, agent, best_rule, prev_best_rule, prompt_trajectories
            )
            # Train the agent
